...
// Config represents the Gorm adapter for policy storage.
type Config struct {
	DriverName     string
...
func New(config Config) (*Adapter, error) {
	...
	if config.PrimaryKey == "" {
		config.PrimaryKey = defaultPrimaryKey
	}
	adapter := &Adapter{
		...
		isFiltered:     config.IsFiltered,
	}
	if config.DB != nil {
		adapter.db = config.DB.Scopes(adapter.casbinRuleTable()).Session(&gorm.Session{Context: config.DB.Statement.Context})
...
func loadPolicyLine(line CasbinRule, model model.Model) error {
	var p = []string{line.Ptype,
		...
		line.V3, line.V4, line.V5, line.V6, line.V7}
...
func (a *Adapter) LoadFilteredPolicy(model model.Model, filter interface{}) error {
	...
	for _, f := range batchFilter.filters {
		if err := a.db.Scopes(a.filterQuery(a.db, f)).Order(a.primaryKey).Find(&lines).Error; err != nil {
			return err
		}
...
func (a *Adapter) Preview(rules *[]CasbinRule, model model.Model) error {
	...
	for i, rule := range *rules {
		r := []string{rule.Ptype,
			...
			rule.V3, rule.V4, rule.V5, rule.V6, rule.V7}
		index := len(r) - 1
		for r[index] == "" {
			index--
		}
